...
const app = $('#app');
const state = { center:null, fields:[], selectedField:null, selectedSlot:null, me:null, manager:null, centers:[], managers:[] };
    ...
    <section class="center-section">
      ...
      <div class="field-grid">${state.fields.length?state.fields.map(f=>`
        <article class="field-premium-card">
          ...
          <div class="field-bottom"><div><strong>${money(f.price_cents)} / ${money(f.shower_price_cents||0)} a persona</strong><span>Senza doccia / con doccia · ${f.duration_minutes} min</span></div><button class="round-action" data-book-field="${f.id}">→</button></div>
...
async function loadSlots(){
  ...
  try{
    ...
    $$('.slot:not(:disabled)').forEach(b=>b.onclick=()=>{
      ...
      $('#selectedTime').innerHTML=`<strong>${esc(state.selectedField.name)} · ${date.split('-').reverse().join('/')} · ${b.dataset.slot}–${b.dataset.end}</strong><br><span>Tariffa: ${money(state.selectedField.price_cents)} a persona senza doccia · ${money(state.selectedField.shower_price_cents||0)} a persona con doccia. Il pagamento avviene direttamente al gestore.</span>`;
...
function managerForm(mgr={}){
  if(!state.centers.length){ toast('Crea prima almeno un centro','error'); return; }
  const editing=!!mgr.id;
  const m=modal(`<div class="section-head"><h2>${editing?'Modifica accesso gestore':'Nuovo accesso gestore'}</h2><button class="btn small" data-close>Chiudi</button></div><form id="managerForm" class="form-grid"><input type="hidden" name="id" value="${mgr.id||''}"><div class="field full"><label>Centro assegnato</label><select class="input" name="center_id">${state.centers.map(c=>`<option value="${c.id}" ${Number(mgr.center_id)===Number(c.id)?'selected':''}>${esc(c.name)}</option>`).join('')}</select></div><div class="field"><label>Nome gestore</label><input class="input" name="name" value="${esc(mgr.name||'')}" required></div><div class="field"><label>Email di accesso</label><input class="input" name="email" type="email" value="${esc(mgr.email||'')}" required></div>${editing?'':`<div class="field full"><label>Password iniziale</label><input class="input" name="password" type="password" minlength="6" required><div class="muted">Almeno 6 caratteri. Potrai cambiarla dalla ADMIN in qualsiasi momento.</div></div>`}${editing?`<label class="check"><input type="checkbox" name="active" ${mgr.active?'checked':''}> Accesso attivo</label>`:''}<div class="field full"><button class="btn primary full-btn">${editing?'Salva modifiche':'Crea accesso gestore'}</button></div></form>`);
  $('[data-close]',m).onclick=()=>m.remove();
  $('#managerForm',m).onsubmit=async e=>{e.preventDefault();const fd=Object.fromEntries(new FormData(e.currentTarget));fd.id=Number(fd.id||0);fd.center_id=Number(fd.center_id);if(editing)fd.active=e.currentTarget.active.checked;try{await api(editing?'admin-manager-update':'admin-manager-create',{method:'POST',body:JSON.stringify(fd)});m.remove();toast(editing?'Accesso aggiornato':'Accesso gestore creato');loadAdmin()}catch(err){toast(err.message,'error')}};
}
function resetManagerPassword(mgr){
  const m=modal(`<div class="section-head"><div><h2>Nuova password</h2><div class="muted">${esc(mgr.name)} · ${esc(mgr.email)}</div></div><button class="btn small" data-close>Chiudi</button></div><form id="resetManagerForm" class="form-grid"><div class="field full"><label>Nuova password</label><input class="input" type="password" name="password" minlength="6" required autofocus></div><div class="field full"><button class="btn primary full-btn">Aggiorna password</button></div></form>`);
  $('[data-close]',m).onclick=()=>m.remove();
  $('#resetManagerForm',m).onsubmit=async e=>{e.preventDefault();const fd=Object.fromEntries(new FormData(e.currentTarget));try{await api('admin-manager-reset-password',{method:'POST',body:JSON.stringify({id:mgr.id,password:fd.password})});m.remove();toast('Password aggiornata. Le sessioni del gestore sono state chiuse.')}catch(err){toast(err.message,'error')}};
}
